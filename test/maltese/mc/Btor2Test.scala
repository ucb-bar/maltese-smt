...
class Btor2Test extends AnyFlatSpec {
  ...
  "single bool state" should "be parsed correctly" in {
    ...
    assert(sys.states.head.sym.name == "test")
    assert(sys.states.head.sym.tpe == BVType(1))
  }

  "single bitvector state" should "be parsed correctly" in {
    val sys = parse(Seq("1 sort bitvec 3", "2 state 1 test"))
    assert(sys.states.head.sym.tpe == BVType(3))
  }

  "single array state" should "be parsed correctly" in {
    val sys = parse(Seq("1 sort bitvec 3", "2 sort array 1 1", "3 state 2 test"))
    assert(sys.states.head.sym.tpe == ArrayType(3, 3))
  }

  def parse_res(src: Seq[String]): SMTExpr = {
    val sys = parse(src, inlineSignals = true)
  ...
  "mul expressions" should "be parsed correctly" in {
    ...
    assert(e == BVOp(Op.Mul, True(), False()))
  }

  // this example if from the official btor2tools repository
  "count2" should "be parsed correctly" ignore { // TODO: fix serializer
    val count2 =
      """
        |1 sort bitvec 3
        |2 zero 1
  ...
  // this example if from the official btor2tools repository
  "factorial4even" should "be parsed correctly" ignore { // TODO: fix serializer
    val fact =
      """
        |1 sort bitvec 4
        |2 one 1
    ...
    val sys = parse(fact)
    val factorial = sys.states.find(_.sym.name == "factorial").get
    val i = sys.states.find(_.sym.name == "i").get

    assert(factorial.sym.name == "factorial")
    assert(factorial.init.contains(BVLiteral(1, 4)))
    assert(factorial.next.contains(BVOp(Op.Mul, factorial.sym.asInstanceOf[BVExpr], i.sym.asInstanceOf[BVExpr])))

    assert(i.sym.name == "i")
    assert(i.init == factorial.init)
    assert(i.next.contains(BVOp(Op.Add, i.sym.asInstanceOf[BVExpr], BVLiteral(1, 4))))

    val bad = sys.signals.filter(_.lbl == IsBad)
    val b0 = bad.head.e
    assert(b0 == BVEqual(i.sym.asInstanceOf[BVExpr], BVLiteral(15, 4)))

    val b1 = bad(1).e
    val ugt = BVComparison(Compare.Greater, i.sym.asInstanceOf[BVExpr], BVLiteral(3, 4), signed = false)
    val slice = BVSlice(factorial.sym.asInstanceOf[BVExpr], 0, 0)
    val slice_b = BVEqual(slice, BVLiteral(1, 1))
    // this is an artifact from the parse, serialize, parse
    val slice_b_artifact = BVEqual(slice_b, True())
    assert(b1 == BVAnd(ugt, slice_b_artifact))
  }

  // this example was written by Kevin Laeufer to demonstrate a bug in cosa2's parsing
  "const_array_example" should "be parsed correctly" ignore { // TODO: fix serializer
    val ca =
      """
        |1 sort bitvec 1
        |5 sort bitvec 5
    ...
    val sys = parse(ca)
    val addr = sys.states.find(_.sym.name == "addr").get
    val data = sys.states.find(_.sym.name == "data").get
    val mem = sys.states.find(_.sym.name == "mem").get
    val mem_n = sys.states.find(_.sym.name == "mem_n").get
    val a = sys.states.find(_.sym.name == "a").get
    val b0 = sys.signals.find(_.lbl == IsBad).get
  ...
  val custom_constraints =
    """
      ...
      |234 constraint 233
      |""".stripMargin
  "custom constraints" should "parse" ignore { // TODO: add support for array type inputs
